    ...
    if !isfile(rulesPath)
        ...
        for y = 0:1
            ...
            while (cmax >= n*mincovy)#et peut etre ajouter la condition itérations
                println("entrée iteration 1, rules:$rules")
                if (iter == 1)
                ...
                @constraint(modele,  sum(b[j] for j in 1:d if rule[j]<0.0000001)+sum(1-b[j] for j in 1:d if rule[j]==1) >= .99999999)
